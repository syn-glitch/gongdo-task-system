...
## 15단계: 통합 지출 증빙 및 영수증 자동 파싱 (Expense & AI Parsing Integration)
...
## 17단계: 역할 기반 업무 뷰 및 팀원 협업 워크플로우 (Role-Based Views & Team Collaboration)
> 📖 상세 계획: [implementation_plan_phase17.md](implementation_plan_phase17.md)
> 📌 **선행 조건**: 16단계 완료 (정형화된 데이터 구조 필수)
...
## 18단계: 일정 중심 캘린더 직접 등록 (Schedule-First Calendar Integration)
> 📖 상세 계획: [implementation_plan_phase18.md](implementation_plan_phase18.md)
> ⚠️ **기존 4단계와의 차이**: 4단계는 업무 마감일 → 종일 이벤트 자동 반영. 18단계는 회의/미팅/외근 → 시간 지정 이벤트 직접 등록
...
## 19단계: 팀 온보딩 가이드 및 AI 피드백·평가 시스템 (Onboarding & Feedback System)
...
## 20단계: 업무 라이프사이클 고도화 — 상태 관리, 시간 트래킹, 회고, 스마트 리마인더 (Task Lifecycle Enhancement)
