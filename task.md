...
## 9단계: 사용자 전면 동적 관리 (Dynamic User List Integration) 
- [ ] **웹 백엔드 (`web_app.gs`) 시트 연동**
  - `Users` 시트 데이터(1열 이름, 2열 슬랙ID)를 파싱 및 JSON화 하여 반환하는 `getUsersFromSheet` 함수 신설
- [ ] **웹 프론트엔드 (`judy_note.html`) 동적 렌더링**
  - 초기 접속 시 시트 데이터를 불러와 `<select id="userSelect">` 내부 옵션을 동적으로 생성하는 로직 구축
- [ ] **슬랙 봇 1시간 캐시 및 시트 연동 (`slack_command.gs`)**
  - `fetchUserName` 함수 내 하드코딩 딕셔너리 제거
  - 타임아웃 방지를 위해 `CacheService`를 활용하여 시트 데이터를 1시간 단위 캐싱 후 맵핑하는 구조 도입
