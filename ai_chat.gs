...
/**
 * 🕒 백그라운드 처리 함수: 사용자의 채팅(질문)을 처리합니다.
 * slack_command.gs 의 doPost() 에서 1초 뒤 실행되도록 트리거된 함수입니다.
 */
function processAiChatAsync(e) {
  // 1. 일회용 트리거 삭제 (쓰레기 방지)
  if (e && e.triggerUid) {
  ...
  // 2. 임시 저장소에서 대기 중인 채팅 데이터를 꺼내옵니다.
  ...
  for (const key in allProps) {
    if (key.startsWith("CHAT_")) {
      try {
        ...
        // 3. Claude API 호출
        const userQuery = data.text.replace(/<@[A-Z0-9]+>/g, "").trim(); // 멘션 태그 제거
        ...
        if (typeof sendSlackMessage === 'function') {
          ...
          sendSlackMessage(data.channel, `🤖 *주디의 답변:*\n\n${aiResponse}`);
        } else {
        ...
        // data가 파싱 실패했을 수도 있으니 에러 방어
        const parsedData = JSON.parse(allProps[key] || "{}");
        if (parsedData.channel && typeof sendSlackMessage === 'function') {
           sendSlackMessage(parsedData.channel, `⚠️ 죄송합니다. 데이터 분석 중 에러가 발생했습니다: ${err.message}`);
        }
      }
