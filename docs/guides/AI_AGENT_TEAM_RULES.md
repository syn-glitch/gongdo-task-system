...
5. [QA 팀 워크플로우](#4-qa-팀-워크플로우) ⭐ NEW (v2.0)
6. [회고 규칙 (정기 회고 체계)](#5-회고-규칙-정기-회고-체계) ⭐ 대폭 개선
7. [디버깅 기록 규칙](#6-디버깅-기록-규칙)
8. [Git 커밋 및 배포 규칙](#7-git-커밋-및-배포-규칙)
9. [추가 운영 규칙](#8-추가-운영-규칙)
...
**v2.0 신규**: 김감사 1인 체제 → **QA 전문 에이전트 팀** (4명) 체제

### 🎯 팀 구성

```
🕵️‍♂️ 김감사 (QA Team Lead & Rules Manager)
    ↓ (작업 분배 및 통합 검토)
┌──────────────┬──────────────┬──────────────┐
│ 🔍 테스터    │ 🛡️ 보안     │ 🎨 UX       │
...
### 4-1. QA 팀 역할 분리 원칙 ✅ 필수

**"Stay in Your Lane"**: 각 QA 전문가는 자신의 영역에만 집중
...
| **🕵️‍♂️ 김감사 (Lead)** | 전체 통합, 최종 승인/반려 | - |

### 4-2. QA 프로세스 (병렬 리뷰) ✅ 필수
...
Step 2: 김감사 - 작업 분배 결정
  ...
  • Frontend 변경 → Tester + UX
  • Full-Stack 변경 → 전체 QA 팀 (3명)
        ↓ (병렬 리뷰 - 동시 진행)
Step 3: QA 전문가들 독립 리뷰 (15분 내)
  ├─ 🔍 테스터: functional_qa.md 작성 (15분)
  ├─ 🛡️ 보안 감사관: security_audit.md 작성 (10분)
  └─ 🎨 UX 검증관: ux_validation.md 작성 (10분)
        ↓
Step 4: 김감사 - 통합 검토 및 최종 판정 (10분)
  • 3개 리포트 읽기 및 취합
  • 중복 이슈 제거
...
Step 5: 팀장 보고 및 배포 결정
  ...
  • ⚠️ 조건부 승인: Medium만 있음 (배포 후 추적)
  • ❌ 반려: Critical/High 이슈 1개 이상
```

### 4-3. QA 시간 제한 ⏱️ 엄격 적용
...
├── qa_reviews/
│   ├── functional/          # 🔍 기능 테스트
│   │   └── YYYY-MM-DD_feature_name_functional_qa.md
│   ├── security/            # 🛡️ 보안 감사
│   │   └── YYYY-MM-DD_feature_name_security_audit.md
│   ├── ux/                  # 🎨 UX 검증
│   │   └── YYYY-MM-DD_feature_name_ux_validation.md
│   └── integrated/          # 🕵️‍♂️ 통합 최종 리포트
│       └── YYYY-MM-DD_feature_name_final_qa_report.md
├── templates/               # QA 템플릿
└── retrospectives/          # QA 회고 (일간/주간/월간)
```
...
**우선순위 정의**:
- 🔴 **Critical**: 기능 중단, 데이터 손실, 보안 취약점 (API 키 노출, 크래시, 인증 우회)
- 🟠 **High**: 주요 기능 심각한 영향, 사용자 경험 매우 나쁨 (에러 핸들링 누락, 모바일 미지원)
- 🟡 **Medium**: 기능 작동하나 불편, 일부 케이스 오작동 (Edge Case 미처리, 느린 응답)
- 🟢 **Low**: 매우 작은 문제, 코드 품질 개선 (변수명 개선, 주석 추가, 로그 개선)

### 4-6. Fast-Track QA (Micro 작업) ⚡

**예상 소요 시간 < 30분인 작업**:
- **QA 팀 축소**: 테스터 1명만 리뷰 또는 김감사 자가 검토
- **시간 제한**: 5분
- **리포트 간소화**: 체크리스트만 확인
- **예시**: 오타 수정, CSS 미세 조정, 문서 업데이트
...
### 4-8. QA 팀 커뮤니케이션 규칙 ✅ 필수

**독립성 보장**: QA 전문가들은 서로 직접 소통하지 않음

- ❌ 테스터가 보안 감사관에게 "이거 보안 문제 아닌가요?" 질문
- ❌ UX 검증관이 테스터에게 "이 기능 작동 안 돼요" 보고
- ✅ 모든 질문/보고는 **김감사에게만**

**이유**: 각자 독립적으로 리포트 작성 → 편향 없는 다각도 분석

**상세 규칙**: [qa/qa_team_rules.md](../../qa/qa_team_rules.md) 참고
